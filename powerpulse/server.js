...
app.post("/api/reports/summary", requireAuth, express.json(), async (req, res) => {
  try {
    const { report, range, startDate, endDate, customPrompt } = req.body;
    if (!report) return res.status(400).json({ error: "Report data is required" });

    const summary = await generateReportSummary(report, range, startDate, endDate, customPrompt);
    res.json({ summary });
  } catch (err) {
...
// Alias for new reports frontend
app.post("/api/reports/ai-summary", requireAuth, express.json(), async (req, res) => {
  try {
    const { reportData, range, startDate, endDate, customPrompt } = req.body;
    if (!reportData) return res.status(400).json({ error: "Report data is required" });

    const summary = await generateReportSummary(reportData, range, startDate, endDate, customPrompt);
    res.json({ summary });
  } catch (err) {
...
app.get("/settings", requireAuth, (req, res) => res.sendFile(path.join(__dirname, "public/html/settings.html")));

// ✅ Settings CRUD APIs
const SETTINGS_PATH = path.join(__dirname, "config", "settings.json");

const DEFAULT_SETTINGS = {
  general: { theme: "dark", refreshInterval: 30, sidebarCollapsed: false },
  charts: { maxDataPoints: 20, yAxisMode: "auto", animation: false, showChartToolbar: true },
  alerts: { voltageMin: 180, voltageMax: 250, currentMax: 100, pfMin: 0.80, thdMax: 10, cooldownSeconds: 300, custom: [] },
  ai: { 
    bedrockApiKey: "", 
    openaiApiKey: "", 
  ...
  reports: { companyName: "PowerPulse", electricityRate: 8, currency: "₹", includeCost: true }
};
...
app.get("/api/settings", requireAuth, (req, res) => {
  ...
  // Mask API keys for non-admin users or general safety
  if (req.user.role !== "admin") {
    if (settings.ai?.bedrockApiKey) {
      const key = settings.ai.bedrockApiKey;
      settings.ai.bedrockApiKey = key.length > 8 ? key.slice(0, 4) + "****" + key.slice(-4) : "****";
    }
    if (settings.ai?.openaiApiKey) {
      const key = settings.ai.openaiApiKey;
      settings.ai.openaiApiKey = key.length > 8 ? key.slice(0, 4) + "****" + key.slice(-4) : "****";
    }
  }
  res.json(settings);
});

app.put("/api/settings", requireAuth, express.json(), (req, res) => {
  try {
    const current = readSettings();
    const incoming = req.body;
    // Merge incoming settings
    if (incoming.general) current.general = { ...current.general, ...incoming.general };
    if (incoming.charts) current.charts = { ...current.charts, ...incoming.charts };
    if (incoming.alerts) current.alerts = { ...current.alerts, ...incoming.alerts };
    if (incoming.reports) current.reports = { ...current.reports, ...incoming.reports };
    // Only admin can update AI settings
    if (incoming.ai && req.user.role === "admin") {
      // Protect real keys — don't overwrite if the incoming value is masked or empty
      if (!incoming.ai.bedrockApiKey || incoming.ai.bedrockApiKey.includes("****")) {
        incoming.ai.bedrockApiKey = current.ai?.bedrockApiKey || "";
      }
      if (!incoming.ai.openaiApiKey || incoming.ai.openaiApiKey.includes("****")) {
        incoming.ai.openaiApiKey = current.ai?.openaiApiKey || "";
      }
      // Strip any legacy fields that may exist in old configs
      delete incoming.ai.provider;
      delete incoming.ai.bedrockModelId;
      delete incoming.ai.openaiModelId;
      current.ai = { ...current.ai, ...incoming.ai };
      // Also strip from current in case they were loaded from old file
      delete current.ai.provider;
      delete current.ai.bedrockModelId;
      delete current.ai.openaiModelId;
    }
...
app.post("/api/settings/reset", requireAuth, (req, res) => {
  try {
    // Preserve both existing API keys on reset
    const current = readSettings();
    const resetSettings = JSON.parse(JSON.stringify(DEFAULT_SETTINGS));
    if (current.ai?.bedrockApiKey) {
      resetSettings.ai.bedrockApiKey = current.ai.bedrockApiKey;
    }
    if (current.ai?.openaiApiKey) {
      resetSettings.ai.openaiApiKey = current.ai.openaiApiKey;
    }
