document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("exportCSVBtn")?.addEventListener("click", () => {
    ...
    const keys = Object.keys(data[0]).filter(k => k !== "pk");
    ...
    const source = dataSourceSelect?.value || "data";
    downloadFile(csv, `PowerPulse_${source}_${new Date().toISOString().slice(0, 10)}.csv`, "text/csv");
  });
  ...
  document.getElementById("exportJSONBtn")?.addEventListener("click", () => {
    ...
    if (!data || !data.length) { showToast("No data to export", "error"); return; }
    const source = dataSourceSelect?.value || "data";
    downloadFile(JSON.stringify(data, null, 2), `PowerPulse_${source}_${new Date().toISOString().slice(0, 10)}.json`, "application/json");
  });
